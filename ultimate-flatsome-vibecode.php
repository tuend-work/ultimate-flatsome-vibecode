...
function vbc_button_shortcode($atts, $content = null) {
    ...
    $btn_text = preg_replace('/<\/?p[^>]*>/i', '', $btn_text); // Strip any <p> tags injected by wpautop inside button text
...
/**
 * 7. ĐĂNG KÝ CÁC PHẦN TỬ CONTAINER VÀ PHẦN TỬ KHÁC VÀO UX BUILDER
 */
function vbc_register_layout_ux_builder() {
    if (!function_exists('add_ux_builder_shortcode')) {
        return;
    }
    ...
    // Đăng ký vbc_div, vbc_box, vbc_block, vbc_container, vbc_card như các Container
    $layout_tags = array(
        'vbc_div' => 'VBC Div',
        'vbc_box' => 'VBC Box',
        'vbc_block' => 'VBC Block',
        'vbc_container' => 'VBC Container',
        'vbc_card' => 'VBC Card',
    );

    foreach ($layout_tags as $tag => $name) {
        add_ux_builder_shortcode($tag, array(
            'name' => $name,
            'category' => 'VibeCode HTML',
            'type' => 'container',
    ...
    // Đăng ký các thẻ văn bản như Container để UX Builder không bọc lung tung thẻ p
    $text_tags = array(
        'vbc_p' => 'VBC Paragraph',
        'vbc_span' => 'VBC Span',
        'vbc_a' => 'VBC Link',
        'vbc_h1' => 'VBC Heading 1',
        'vbc_h2' => 'VBC Heading 2',
        'vbc_h3' => 'VBC Heading 3',
        'vbc_h4' => 'VBC Heading 4',
        'vbc_h5' => 'VBC Heading 5',
        'vbc_h6' => 'VBC Heading 6',
        'vbc_ul' => 'VBC Unordered List',
        'vbc_ol' => 'VBC Ordered List',
        'vbc_li' => 'VBC List Item',
    );

    foreach ($text_tags as $tag => $name) {
        add_ux_builder_shortcode($tag, array(
            'name' => $name,
            'category' => 'VibeCode HTML',
            'type' => 'container',
            'options' => array_merge($common_options, array(
                'font_size' => array(
                    'type' => 'textfield',
                    'heading' => 'Font Size',
                    'default' => '',
    ...
    // Đăng ký vbc_img
    add_ux_builder_shortcode('vbc_img', array(
        'name' => 'VBC Image',
        'category' => 'VibeCode HTML',
        'options' => array(
            'img_source' => array(
...
add_action('ux_builder_setup', 'vbc_register_layout_ux_builder');
